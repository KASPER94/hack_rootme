...
OUTPUT_DIR = 'program_hack/adversarial_attack_gan/solutions/robust_all'
...
def robust_loss(img_var, model, target_class):
    ...
    # Simulate Min-Max scaling variations
    img_scaled = (img_var * 2.0) - 1.0
    pred_scaled = model(img_scaled)
    
    # Average logits for robustness
...
def attack_class(model, source_img, target_class, steps=1500): # More steps
    ...
    for step in range(steps):
        ...
        gradients = tf.sign(gradients) # FGSM
        
        # Slightly stronger steps for harder classes
        img_var.assign_add(-gradients * 0.0008) 
...
def main():
    ...
    # Done: 2, 8, 9, 10
    remaining_classes = [0, 1, 3, 4, 5]
    
    for cls in remaining_classes:
        final_img = attack_class(model, source_img, cls, steps=1500)
    ...
    print(f"\n[*] READY TO TEST:")
    for cls in remaining_classes:
        print(f"curl -k -F 'image=@{OUTPUT_DIR}/robust_class_{cls}.jpg' https://programmation-ch5.challenge01.root-me.org/authenticate")
